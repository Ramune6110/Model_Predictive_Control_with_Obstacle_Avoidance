...
for i = 1:sim_step
    ...
    %% Solve CFTOC    
    [~, uk_one] = solveMPCDC(x_curr_one, system, params_mpc_dc, obs2, obs3, obs4, target1);
    [~, uk_two] = solveMPCDC(x_curr_two, system, params_mpc_dc, obs1, obs3, obs4, target2);
    [~, uk_three] = solveMPCDC(x_curr_three, system, params_mpc_dc, obs1, obs2, obs4, target3);
    [~, uk_four] = solveMPCDC(x_curr_four, system, params_mpc_dc, obs1, obs2, obs3, target4);
...
function [xopt, uopt] = solveMPCDC(xk, system, params_mpc_dc, obs1, obs2, obs3, target)
    % Solve MPC-DC
    [feas, x, u, J] = solve_cftoc(xk, system, params_mpc_dc, obs1, obs2, obs3, target);
    if ~feas
        xopt = [];
        uopt = [];
        return
...
function [feas, xopt, uopt, Jopt] = solve_cftoc(xk, system, params, obs1, obs2, obs3, target)
    % Solve CFTOC
    % extract variables
    N = params.N;
    ...
    for i = 1:N
        ...
        b_next = (x([1:2],i+1) - pos)'*((x([1:2],i+1) - pos)) - (1.0 * r)^2;
