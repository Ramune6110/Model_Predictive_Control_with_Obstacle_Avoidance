...
for i = 1:sim_step
    ...
    % Solve CFTOC    
    [~, uk_one] = solveMPCDC(x_curr_one, x_curr_two, system, params_mpc_dc, obs1, target1);
    [~, uk_two] = solveMPCDC(x_curr_two, x_curr_one, system, params_mpc_dc, obs2, target2);
...
function [xopt, uopt] = solveMPCDC(xk, opponent_xk, system, params_mpc_dc, obs, target)
    % Solve MPC-DC
    [feas, x, u, J] = solve_cftoc(xk, opponent_xk, system, params_mpc_dc, obs, target);
    if ~feas
        xopt = [];
        uopt = [];
        return
...
function [feas, xopt, uopt, Jopt] = solve_cftoc(xk, opponent_xk, system, params, obs, target)
    % Solve CFTOC
    % extract variables
    N = params.N;
