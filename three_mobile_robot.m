...
% while time_curr <= time
    ...
    %% Solve CFTOC    
    [~, uk_one] = solveMPCDC(x_curr_one, system, params_mpc_dc, obs2, obs3, target1);
    [~, uk_two] = solveMPCDC(x_curr_two, system, params_mpc_dc, obs1, obs3, target2);
    [~, uk_three] = solveMPCDC(x_curr_three, system, params_mpc_dc, obs1, obs2, target3);
...
%     u_cost = u_cost + uk' * uk * system.dt;
    ...
    Animation(xlog_one, xlog_two, xlog_three, obs1, obs2, obs3, i);
end
...
function [xopt, uopt] = solveMPCDC(xk, system, params_mpc_dc, obs1, obs2, target)
    % Solve MPC-DC
    [feas, x, u, J] = solve_cftoc(xk, system, params_mpc_dc, obs1, obs2, target);
    if ~feas
        xopt = [];
        uopt = [];
        return
...
function [feas, xopt, uopt, Jopt] = solve_cftoc(xk, system, params, obs1, obs2, target)
    % Solve CFTOC
    % extract variables
    N = params.N;
...
function Animation(xlog_one, xlog_two, xlog_three, obs1, obs2, obs3, i)
...
%     r = 0.4;
    th = linspace(0,2*pi*100);
    x = cos(th) ; y = sin(th) ;
    plot(pos3(1) + r*x, pos3(2) + r*y, 'Color', [0.8500, 0.3250, 0.0980],'LineWidth', 2);
%     plot(pos2(1), pos2(2), 'Color', [0.8500, 0.3250, 0.0980],'MarkerSize', 5, 'LineWidth', 2);
